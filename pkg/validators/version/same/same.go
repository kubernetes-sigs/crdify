...
func (v *Validator) Validate(old, new *apiextensionsv1.CustomResourceDefinition) map[string]map[string][]validations.ComparisonResult {
	result := map[string]map[string][]validations.ComparisonResult{}
	for _, oldVersion := range old.Spec.Versions {
		newVersion := validations.GetCRDVersionByName(new, oldVersion.Name)

...
func (v *Validator) Validate(_, new *apiextensionsv1.CustomResourceDefinition) map[string]map[string][]validations.ComparisonResult {
	...
	if v.conversionPolicy == config.ConversionPolicyIgnore && new.Spec.Conversion != nil && new.Spec.Conversion.Strategy == apiextensionsv1.WebhookConverter {
		return result
	}
